...
cm.MIDIReceived.argtypes = [c_void_p, c_void_p]
cm.MIDIReceived.restype = c_int
...
def midi_send((c, p, s), packets):
    plist = midi_list_size(len(packets))()
    ppacket_ref = cm.MIDIPacketListInit(byref(plist))
    for p in packets:
        lp = len(p)
        ppacket_ref = cm.MIDIPacketListAdd(byref(plist), sizeof(plist), ppacket_ref, 0, lp, p)
    cm.MIDIReceived(s, byref(plist))
